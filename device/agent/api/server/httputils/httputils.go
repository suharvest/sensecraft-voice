...
import (
	...
	validatorutil "github.com/YOUR-ORG/sensecraft-voice/device/agent/api/server/validator"
)
...
// e.g. /api/v1/clusters/1 -> "clusters" "1" true
func getObjectFromRequest(path string) (obj, sid string, ok bool) {
	...
	l = len(subs)
	if l < 2 || subs[0] != "sensecraftVoice" {
		return
	}
	if l == 2 {
		// e.g. /api/v1/clusters -> "clusters" "" true
